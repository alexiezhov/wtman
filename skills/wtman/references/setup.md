...
```bash
wtman version 2>/dev/null   # prints version if installed; silent if not
brew --version 2>/dev/null   # Homebrew is the recommended install path
git --version
...
- If `wtman version` printed a version → skip to **Step 2** (config).
- If it printed nothing → wtman isn't installed; start at **Step 1** (install).

If `sw_vers` shows something other than `macOS`, warn the user: wtman ships as a macOS Homebrew cask, and its `post_command` defaults and the watcher rely on macOS-only tools (`open`, `osascript`, `script`).
...
- **git** — `git --version` to check. Required at runtime for all worktree operations. Should always be present on a developer machine; if not, `brew install git`.
- **Homebrew** — `brew --version`. The install path for wtman. If it's missing, point the user at [brew.sh](https://brew.sh) to install it; don't install Homebrew for them.

If a prerequisite is missing, stop and tell the user — don't try to work around it.
...
This taps `alexiezhov/homebrew-apps` and installs the latest release into a Homebrew-managed location already on `PATH` — no manual `PATH` step needed. Upgrade later with `brew upgrade --cask wtman`.
...
If it prints the version, commit, and build date, the install is good — proceed to step 2. (`wtman version` runs before any config load, so it works even with no config yet.)
